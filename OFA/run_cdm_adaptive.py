"""
run_cdm_adaptive.py — Adaptive-depth GIN variant of run_cdm.py.

Changes from run_cdm.py (original is untouched):
  • BinGraphAttModel → AdaptiveGINModel
  • BinGraphModel    → AdaptiveGINModel
  • PyGRGCNEdge      → _DummyGNN  (instantiated by main() but immediately
                                   discarded; AdaptiveGINModel builds its own)

Use with:
  python run_cdm_adaptive.py \\
...
# ── Patch run_cdm BEFORE its main() is called ────────────────────────────────
import run_cdm
from models.model_adaptive import AdaptiveGINModel

class _DummyGNN(nn.Module):
    """Placeholder — instantiated by run_cdm.main() but never used."""
    def forward(self, g): pass

run_cdm.PyGRGCNEdge      = lambda *a, **kw: _DummyGNN()
run_cdm.BinGraphAttModel = AdaptiveGINModel
run_cdm.BinGraphModel    = AdaptiveGINModel
# ─────────────────────────────────────────────────────────────────────────────
...
if __name__ == "__main__":
    ...
    print(params)
    run_cdm.main(params)   # original main() — sees patched BinGraphAttModel
