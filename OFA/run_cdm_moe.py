"""
run_cdm_moe.py — Topology-Aware Graph MoE variant of run_cdm.py.

Patches:
  BinGraphAttModel → GraphMoEModel
  BinGraphModel    → GraphMoEModel
  training_step    → adds AUX_WEIGHT * model.last_aux_loss to ib_loss

Original run_cdm.py is untouched.

Run command:
  python run_cdm_moe.py \\
...
# ── Patch model and training step BEFORE importing run_cdm ───────────────────
import run_cdm
from models.model_moe import GraphMoEModel

run_cdm.BinGraphAttModel = GraphMoEModel
run_cdm.BinGraphModel    = GraphMoEModel
run_cdm.PyGRGCNEdge      = lambda *a, **kw: nn.Module()  # placeholder

# Patch training_step to add load-balancing auxiliary loss
from gp.lightning.module_template import IBBaseTemplate

_orig_training_step = IBBaseTemplate.training_step

def _moe_training_step(self, batch, batch_idx, dataloader_idx=0):
    ...
    if (ib_loss is not None
        ...
        ib_loss = ib_loss + GraphMoEModel.AUX_WEIGHT * self.model.last_aux_loss
        self.model.last_aux_loss = None
    return ib_loss

IBBaseTemplate.training_step = _moe_training_step
# ─────────────────────────────────────────────────────────────────────────────
